...
const PrimaryButton = ({ image, btnText, imageAlt, classBtn, classText, fontAwesome }) => {
    return (
        <div>
            <button className={`button-primary py-[12px] px-[5px] sm:px-[40px] relative mr-5 overflow-hidden font-medium text-gray-600 bg-white border border-gray-100 rounded-lg shadow-inner group ${classBtn}`}>
                ...
                <span className={`relative transition-colors duration-300 delay-200 group-hover:text-white ease flex items-center w-full text-[12px] sm:text-[16px] ${classText}`}>{fontAwesome ? fontAwesome : ""} <p className='w-full px-4 sm:px-0'>{btnText}</p> {image && <img className='h-[13px] sm:h-[15px] ml-1 mr-3 sm:mr-0 sm:ml-4' src={image} alt={imageAlt}></img>}</span>
            </button>
        </div>
    );
};
